export default async function handler(req, res) {
  ...
  if (!serverKey) {
    return res.status(500).json({
      error: "MIDTRANS_SERVER_KEY belum dipasang di Vercel"
    });
  }
  ...
  try {
    ...
    if (!name || !email || !whatsapp) {
      return res.status(400).json({ error: "Data pembeli belum lengkap" });
    }

    const orderId =
      `KENZO-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    ...
    const response = await fetch(
      "https://app.sandbox.midtrans.com/snap/v1/transactions",
      {
        ...
        body: JSON.stringify({
          ...
          customer_details: {
            first_name: String(name).slice(0, 50),
            email: String(email).slice(0, 100),
            phone: String(whatsapp).slice(0, 30)
          }
        })
      }
    );
    ...
    return res.status(500).json({
      error: "Server error saat membuat transaksi"
    });
  }
}
